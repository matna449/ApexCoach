...
| load_calculator | activity_type (str), duration_minutes (float), avg_hr_bpm (float, HR-based only), grade_pct (float), rpe (int, RPE-based only) | Session Load Score in AU (float) | Unit — HR-based (flat vs hilly runs), RPE-based, and duration-based session types, per docs/adr/0017 |
